...
describe "controlled vocabulary management", :type => :request do
  ...
  before(:all) do
    @vocab = LafayetteConcerns::Vocabulary.new('http://authority.localhost.localdomain/ns/quxVocabulary2')
    @vocab.persist!
  end
  ...
  describe 'replacing the attributes of a vocabulary with no terms' do
    ...
    it "replacing the attributes of a vocabulary with no terms" do
      ...
      expect(JSON.parse(response.body)).to include('uri' => "http://authority.localhost.localdomain/ns/quxVocabulary2")
  ...
  describe 'replacing the attributes and terms of a vocabulary' do
    ...
    it "replacing the attributes and terms of a vocabulary" do
      ...
      expect(response).to render_template(:update)
      expect(JSON.parse(response.body)).to include('uri' => "http://authority.localhost.localdomain/ns/quxVocabulary")
      expect(JSON.parse(response.body)).to include('label' => ['replaced label'])
      expect(JSON.parse(response.body)).to include('terms' => [{ 'uri' => 'http://authority.localhost.localdomain/ns/quxVocabulary/testTerm', 'label' => ['replaced term label'], 'alt_label' => [], 'pref_label' => [], 'hidden_label' => [] }])
    end
  end
  ...
  describe 'updating the terms for a vocabulary' do
    ...
    it "updates the labels and alt. labels for terms in a vocabulary" do
      ...
      expect(JSON.parse(response.body)).to include('terms' => [{ 'uri' => 'http://authority.localhost.localdomain/ns/quxVocabulary/testTerm', 'label' => ['updated term label'], 'alt_label' => [], 'pref_label' => [], 'hidden_label' => [] }])
    end
  end
end
